...
Table~\ref{tab:main-results} reports mean results over the 15
...
admits noise outliers; resolving this mechanism requires per-corruption
admission traces.
...
\begin{table}[t]
    ...
    scoring with percentile calibration; R3 applies ACI to energy; R4 corrects
    replay-loss scaling.}
...
The contrasting results prevent a universal claim for energy scoring.
Per-corruption ID-rejection and OOD-admission rates would help determine
whether the low CIFAR-10-C AUC arises from score overlap, threshold
calibration, or changes in the samples admitted to memory.
...
This result concerns the reported energy+ACI configuration rather than conformal
calibration in general. Per-corruption admission statistics and rejection-rate
trajectories would help determine whether the controller changes admission
behavior despite similar aggregate metrics.
...
memory occupancy, loss magnitude, and admission rates would be needed to
confirm that the improvement occurs during warm-up rather than through an
uncontrolled experimental difference.
