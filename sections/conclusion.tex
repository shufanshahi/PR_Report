...
The most consequential completed intervention was correcting replay-loss
scaling while memory is partially filled. STAMP+BSN achieved the best completed
H-scores: 81.19 on CIFAR-10-C with noise outliers and 72.89 on CIFAR-100-C.
...
per-corruption metrics, or memory-admission traces. Entropy+ACI remains a
pending comparator. Future work should complete that comparison, log replay
occupancy, loss scale, ID rejection, and OOD admission over time, and test the
replay correction across multiple seeds, semantic OOD sources, and larger
